...
paths.set_mode("host")
import armory.data.datasets as armory_datasets
...
def cifar_loaders(batch_size, shuffle_train = True, shuffle_test = False, train_random_transform = False, normalize_input = False, num_examples = None, test_batch_size=None): 
    ...
    train_loader.std = std
    test_loader.std = std
    train_loader.mean = [0.4914, 0.4822, 0.4465]
    test_loader.mean = [0.4914, 0.4822, 0.4465]
...
def so2sat_loaders(batch_size, shuffle_train=True, shuffle_test=False, train_random_transform=False,
    ...
    train_loader = armory_datasets.so2sat(split='train',epochs=1,batch_size=32)
    test_loader = armory_datasets.so2sat(split='validation',epochs=1,batch_size=32)

    train_loader.std = [1.]*14
    test_loader.std = [1.]*14
    train_loader.mean = [0.]*14
    test_loader.mean = [0.]*14
